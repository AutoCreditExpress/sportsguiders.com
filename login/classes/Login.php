...
class Login
{
    ...
    private function loginWithSessionData()
    {
        ...
        $this->user_first = $_SESSION['user_first'];
        $this->user_last = $_SESSION['user_last'];
        ...
        if (isset($_COOKIE['rememberme'])) {
            ...
            if ($hash == hash('sha256', $user_id . ':' . $token . COOKIE_SECRET_KEY) && !empty($token)) {
                ...
                if ($this->databaseConnection()) {
                    ...
                    $sth = $this->db_connection->prepare("SELECT user_id, user_name, user_first, user_last, user_email FROM users WHERE user_id = :user_id
                    ...
                    if (isset($result_row->user_id)) {
                        ...
                        $_SESSION['user_first'] = $result_row->user_first;
                        ...
                        $this->user_first = $result_row->user_first;
                        $this->user_last = $result_row->user_last;
                        $this->user_is_logged_in = true;
